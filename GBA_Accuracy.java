...
public class GBA_Accuracy
{
    ...
    void Search(int p, int k0, int k, long cap, long start)
    {
        if(System.nanoTime()-start>=timer)
            return;
        int i;
        ...
        for(i=k; i>=0 && !found; i--)
        {
            ...
            if(c==0)
            {
                ...
                Search(p+1, k, binarySearch(i-1, c), c, start);
    ...
    public static void main()
    {
        ...
        {
            GBA_Accuracy ob = new GBA_Accuracy(n);
            ...
            int k = ob.s.length-1, wrong=0;
            for(int i=1; i<=cap; i++)
            {
                ob.timer = 3600;
                long start = System.nanoTime(); //Timer is started.
                ob.Search(0, 0, k, i, start);
                boolean f1 = ob.found;
                ob.timer = 2000000000;
                ob.Search(0, 0, k, i, start);
                if(f1!=ob.found)
            ...
            System.out.println(100 - (double)100.0*wrong/cap); //Time taken to find the solution in Nanoseonds is printed.
        }
    }
}
